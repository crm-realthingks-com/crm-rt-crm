...
export const validateValues = (record: any, tableName: string): { isValid: boolean; errors: string[] } => {
  ...
  for (const [key, value] of Object.entries(record)) {
    ...
    const validatedValue = validateValue(key, value as string, tableName);
    if (validatedValue === null && key in config.enums) {
...
export const createValueValidator = (tableName: string) => {
  return (key: string, value: string) => validateValue(key, value, tableName);
};
