...
interface MeetingActionItemsModalProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  meeting: Meeting | null;
}

export const MeetingActionItemsModal = ({ open, onOpenChange, meeting }: MeetingActionItemsModalProps) => {
  ...
  const fetchActionItems = async () => {
    ...
    try {
      ...
      if (error) {
        ...
        if (error.code === '42P01') {
          console.log('meeting_action_items table does not exist yet');
          setActionItems([]);
          return;
        }
        throw error;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!meeting || !nextAction.trim()) return;
    ...
    try {
      ...
      if (editingItem) {
        ...
        await logUpdate('meeting_action_items', editingItem.id, actionItemData, editingItem);
        ...
        await logCreate('meeting_action_items', data.id, actionItemData);
      ...
      toast({
        title: "Error",
        description: error.message || "Failed to save action item",
        variant: "destructive"
      });
    }
  };
  ...
  const handleDelete = async (item: MeetingActionItem) => {
    try {
      ...
      await logDelete('meeting_action_items', item.id, item);
